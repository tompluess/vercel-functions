...
- [`api/bexio_token_provider.py`](api/bexio_token_provider.py) / [`api/kv_client.py`](api/kv_client.py) — OAuth2 token management. Bexio rotates the refresh token on every refresh, so the token state lives in an Upstash Redis blob (`bexio:oauth`); the provider caches the access token, refreshes under a KV lock, and persists the rotated refresh token. Seed the initial token once with [`scripts/bexio_oauth_bootstrap.py`](scripts/bexio_oauth_bootstrap.py).
...
| `KV_REST_API_URL` | Upstash Redis REST endpoint (Vercel Marketplace integration). Durable store for the rotating Bexio OAuth token (`bexio:oauth`). |
