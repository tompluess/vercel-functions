...
**Telegram error notifications**: ports the n8n `Error Trigger → Send Error to Telegram` handler. `index.py`'s `_notify_failure` fires a Telegram message on every **5xx** path (502 upstream errors, 500 unexpected errors) for all four endpoints, naming the endpoint path, event, source entity id, and error detail. 4xx auth/validation rejections and the moco-sync delete 404 (`TargetNotFoundError`) are intentionally NOT notified — they're webhook noise / a benign mismatch already visible in Moco's delivery log, not application errors. Additionally, `BexioExpenseSyncService`'s three skip branches (no company, no account, bill-not-DRAFT) DM the chat with entity context + the Moco purchase deep-link, mirroring the n8n `…Notification to Telegram` nodes. The expense `telegram` collaborator is optional (defaults to `None`) so service unit tests can omit it; all sends route through `TelegramNotifier.notify`, which never raises.
