...
Required env vars are listed in `REQUIRED_ENV_MOCO_SYNC` / `REQUIRED_ENV_BEXIO_SYNC` / `REQUIRED_ENV_BREVO_SYNC` in `api/index.py` — the handler fails fast with a 500 if any are missing. All collaborators are instantiated per-request inside `index.py`; they don't share state across invocations.
