...
- `api/moco_api.py` — `MocoAPI`: thin typed wrapper around the Moco REST endpoints we use (`list_projects`, `list_activities`, `create_activity`, `update_activity`, `delete_activity`). Owns base-URL construction, auth headers, urllib transport, and X-Total pagination logging. No business logic.
- `api/moco_sync_service.py` — `MocoSyncService`: resolves source project/task names against the target account (falls back to configured defaults when no match), builds the activity payload, and dispatches to `sync_create` (create), `sync_update` (lookup-by-date → update, or create as upsert), or `sync_delete` (lookup-by-date → delete, or raise). Takes a `MocoAPI` collaborator in its constructor — all HTTP transport is delegated, so the service contains only business logic and is unit-tested with a `FakeMocoAPI`.
...
All three collaborators are instantiated per-request inside `index.py`; they don't share state across invocations. Required env vars are listed in `REQUIRED_ENV` in `api/index.py` — the handler fails fast with a 500 if any are missing.
...
Tests live in `tests/`; JSON fixtures under `tests/fixtures/` mirror real Moco Activity webhook bodies, target `/projects` responses, and target `/activities?from=&to=` listings used by the update lookup. `tests/test_sync_service.py` injects a `FakeMocoAPI` directly into the service so no HTTP is involved. `tests/test_moco_api.py` and `tests/test_endpoint.py` exercise the real `MocoAPI` with `urllib.request.urlopen` stubbed in `tests/conftest.py` — no network is touched. CI runs the same `pytest` command on push and PR via `.github/workflows/test.yml`.
